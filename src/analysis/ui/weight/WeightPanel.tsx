...
//
// The shared provider supplies the authored book, two-pass evaluation, and queried geometry.
// This feature does not know whether those answers came from a Camber sweep, a mesh, or
...
import { spreadRegion } from "../../../core/sheet/points";
...
function WeightPanelContents() {
  const {
    ...
    dispatch,
    hull,
    outlines: outlineQuery,
    weight,
    metrics: metricQuery,
    cuts: cutQuery,
  } = useAnalysis();
  const outlines =
    outlineQuery.status === "available" ? outlineQuery.value : null;
  ...
  const { measurements, results } = weight;
  const apply = (command: SheetCommand) => {
  ...
  const send = (command: SheetCommand) => {
    if (
    ...
    <div className="weightpanel">
      ...
      <div className="wpanes">
        ...
        <div className="wmain">
          ...
          <div className="whint" role="status">
            {metricQuery.status === "pending" || cutQuery.status === "pending"
              ? "Updating hull measurements…"
              : metricQuery.status === "error" ||
                  metricQuery.status === "unavailable"
                ? `Hull measurements: ${metricQuery.reason}`
                : cutQuery.status === "error" ||
                    cutQuery.status === "unavailable"
...
function GeometryEditor({
  ...
  if (!outlines)
    return (
      <p className="whint">
        Hull outlines are not available yet, so there is nothing to place a
        point against. The coordinates beside this still evaluate.
      </p>
    );
  return (
    <PointViews
