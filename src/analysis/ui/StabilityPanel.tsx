...
import { scaleStability, conditionFromSheet } from "../stabilityData";
import type { QueryState } from "./useAnalysisQuery";
import {
  ChartFrame,
  PLOT_LEFT_INSET,
  type ChartScale,
...
export interface StabilityPanelProps {
  ...
  readonly sheetResults: BookResults;
}
...
export function StabilityPanel({
  ...
  const data = stability.status === "available" ? stability.value : null;
  const analysis = useMemo(
    () => (data ? scaleStability(data, 1 / metres) : null),
    [data, metres],
  );
  const error =
    stability.status === "error" || stability.status === "unavailable"
      ? stability.reason
      : null;
  ...
  const tonsPerVolume = metres ** 3 * density;
  ...
  const fromSheet = useMemo(
    () => conditionFromSheet(sheetResults, density, metres),
    [sheetResults, density, metres],
  );
  ...
  const imoChecks = useMemo(
    () => createImoChecks(curves, limit, metres),
    [curves, limit, metres],
  );
  ...
  const imoVerdicts: readonly ImoVerdict[] = imoChecks.map((check) => {
    ...
    return criterionVerdict(value, check.min, range);
  });
