...
varying vec3 vN; varying vec3 vW; varying float vWZ;
uniform vec3 uLight,uView,uBase; uniform float uStripes,uAlpha,uWaterZ,uPaint,uBoot; uniform int uZebra,uFlat;
void main(){
  ...
  if(uZebra==1){
    ...
    // surface reads. A soft boot-top (smoothstep over ±uBoot, a fraction of the hull's length — the model's
    // coordinates are absolute now, so a fixed millimetre band would not survive a change of unit) avoids an
    // aliased paint line; uPaint gates it off.
    float sub=(1.0 - smoothstep(uWaterZ-uBoot, uWaterZ+uBoot, vWZ)) * uPaint;
...
// thin camera-facing ribbon (GL line width is unreliable), starboard plus its port mirror, nudged toward the
// eye by BIAS so it sits just proud of the surface without z-fighting.
...
function buildTransomMesh(model: Model, edge: Vec3[]): Mesh {
  if (edge.length < 2)
...
// back to the model's nominal hull box when no mesh has been built yet (e.g. the view is in a lines-plan
// mode) — which is why this needs the model: the box is a proportion of the hull's own length now.
export function getHullBBox(model: Model): number[] {
  return (meshBBox ?? nominalBox(model)).slice();
}
...
// LOA) at a reference orientation, so it depends only on the canvas size and the hull's length — not on the
// live rotation, the edited geometry, or the rake. The live hull then just sits inside that fixed frame,
// centered. (v1 could hard-code the box against its fixed L = 1000; absolute coordinates can't.)
const REF_YAW = -0.62,
  REF_PITCH = 0.42;
const nominalBox = (model: Model): number[] => {
...
function drawLines(
  ...
  if (rebuild || !linesGrid)
    linesGrid = trimmedHullGrid(model, LINES_NS, LINES_R);
  const { grid, creaseCols, keel } = linesGrid;
  svg.replaceChildren();
  // the rows are FULL WIDTH: column 0 the starboard sheer, NC the port sheer, KEEL the centre. v1 drew a
  // starboard half and mirrored it here; the grid carries both halves as one row now, so one pass covers
  // the whole hull and the keel needs no seam.
  ...
  for (let i = 0; i < NS; i++)
    for (let j = 0; j < NC; j++) {
        ...
        D = SP[i + 1][j];
      {
        ...
        if (feature(j)) bold.push([D, A]); // sheer / keel / chine longitudinal
  ...
  // The selected station point's longitudinal (the locus it sweeps) is drawn occluded like everything else:
...
export function draw3d(
  ...
  gl.uniform1f(loc.uDepth, 0.75 * len); // depth-range scale, in the hull's own length (750 at the old L=1000)
  ...
  gl.uniform1f(loc.uBoot, 0.004 * len); // the boot-top's soft band, in the hull's own length (4 at the old L=1000)
  ...
  if (curvCache && curvCache.length) {
    ...
    const hwCurve = (SHEER_HALF_PX * dpr) / pxScale,
      ...
      bias = 0.009 * len; // world-space nudge toward the eye, in the hull's own length
    for (const cc of curvCache) {
      if (cc.curve.length < 2) continue;
      const curves: Vec3[][] = [cc.curve];
      if (cc.mirror) curves.push(cc.curve.map(mir));
      drawMesh(gl, ribbonMesh(curves, view, hwCurve, bias), cc.rgb);
      if (cc.hairs.length) {
        ...
        drawMesh(gl, ribbonMesh(comb, view, hwComb, bias), cc.rgb); // same colour as the base curve
      }
    }
